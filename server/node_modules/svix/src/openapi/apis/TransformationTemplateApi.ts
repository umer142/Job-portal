...
export class TransformationTemplateApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async v1TransformationTemplateCreate(connectorIn: ConnectorIn, idempotencyKey?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'connectorIn' is not null or undefined
        if (connectorIn === null || connectorIn === undefined) {
            throw new RequiredError("TransformationTemplateApi", "v1TransformationTemplateCreate", "connectorIn");
        }
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(connectorIn, "ConnectorIn", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
