...
export class AuthenticationApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param idempotencyKey The request\&#39;s idempotency key
     */
    public async v1AuthenticationStreamPortalAccess(streamId: string, streamPortalAccessIn: StreamPortalAccessIn, idempotencyKey?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'streamId' is not null or undefined
        if (streamId === null || streamId === undefined) {
            throw new RequiredError("AuthenticationApi", "v1AuthenticationStreamPortalAccess", "streamId");
        }


        // verify required parameter 'streamPortalAccessIn' is not null or undefined
        if (streamPortalAccessIn === null || streamPortalAccessIn === undefined) {
            throw new RequiredError("AuthenticationApi", "v1AuthenticationStreamPortalAccess", "streamPortalAccessIn");
        }



        // Path Params
        const localVarPath = '/api/v1/auth/stream-portal-access/{stream_id}'
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(streamPortalAccessIn, "StreamPortalAccessIn", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
...
export class AuthenticationApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to v1AuthenticationStreamPortalAccess
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async v1AuthenticationStreamPortalAccessWithHttpInfo(response: ResponseContext): Promise<HttpInfo<AppPortalAccessOut >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: AppPortalAccessOut = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
