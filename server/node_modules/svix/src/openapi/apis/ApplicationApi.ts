...
export class ApplicationApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param idempotencyKey The request\&#39;s idempotency key
     */
    public async v1MessageExpungeAllContents(appId: string, idempotencyKey?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'appId' is not null or undefined
        if (appId === null || appId === undefined) {
            throw new RequiredError("ApplicationApi", "v1MessageExpungeAllContents", "appId");
        }



        // Path Params
        const localVarPath = '/api/v1/app/{app_id}/msg/expunge-all-contents'
            .replace('{' + 'app_id' + '}', encodeURIComponent(String(appId)));
...
export class ApplicationApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to v1MessageExpungeAllContents
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async v1MessageExpungeAllContentsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<ExpungAllContentsOut >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("202", response.httpStatusCode)) {
            const body: ExpungAllContentsOut = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "ExpungAllContentsOut", ""
            ) as ExpungAllContentsOut;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("400", response.httpStatusCode)) {
            const body: HttpErrorOut = ObjectSerializer.deserialize(
