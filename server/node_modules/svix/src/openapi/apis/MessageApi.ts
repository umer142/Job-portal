...
export class MessageApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param idempotencyKey The request\&#39;s idempotency key
     */
    public async v1MessagePrecheck(appId: string, messagePrecheckIn: MessagePrecheckIn, idempotencyKey?: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'appId' is not null or undefined
        if (appId === null || appId === undefined) {
            throw new RequiredError("MessageApi", "v1MessagePrecheck", "appId");
        }


        // verify required parameter 'messagePrecheckIn' is not null or undefined
        if (messagePrecheckIn === null || messagePrecheckIn === undefined) {
            throw new RequiredError("MessageApi", "v1MessagePrecheck", "messagePrecheckIn");
        }



        // Path Params
        const localVarPath = '/api/v1/app/{app_id}/msg/precheck/active'
            .replace('{' + 'app_id' + '}', encodeURIComponent(String(appId)));
        ...
        requestContext.setHeaderParam("Content-Type", contentType);
        const serializedBody = ObjectSerializer.stringify(
            ObjectSerializer.serialize(messagePrecheckIn, "MessagePrecheckIn", ""),
            contentType
        );
        requestContext.setBody(serializedBody);
...
export class MessageApiResponseProcessor {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async v1EventsPublicWithHttpInfo(response: ResponseContext): Promise<HttpInfo<PollingEndpointOut >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: PollingEndpointOut = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "PollingEndpointOut", ""
            ) as PollingEndpointOut;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("400", response.httpStatusCode)) {
            const body: HttpErrorOut = ObjectSerializer.deserialize(
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to v1MessagePrecheck
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async v1MessagePrecheckWithHttpInfo(response: ResponseContext): Promise<HttpInfo<MessagePrecheckOut >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: MessagePrecheckOut = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
                "MessagePrecheckOut", ""
            ) as MessagePrecheckOut;
            return new HttpInfo(response.httpStatusCode, response.headers, response.body, body);
        }
        if (isCodeInRange("400", response.httpStatusCode)) {
            const body: HttpErrorOut = ObjectSerializer.deserialize(
